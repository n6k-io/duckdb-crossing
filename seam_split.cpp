...
SeamRow InsertRow(LogicalInsert &insert, unique_ptr<LogicalOperator> feed, const FreshTableIndex &fresh) {
...
SeamRow SeamRowOf(LogicalOperator &write, const vector<column_t> &key_columns, unique_ptr<LogicalOperator> feed,
	...
	case LogicalOperatorType::LOGICAL_INSERT:
		return InsertRow(write.Cast<LogicalInsert>(), std::move(feed), fresh);
	case LogicalOperatorType::LOGICAL_UPDATE:
		return UpdateRow(write.Cast<LogicalUpdate>(), key_columns, std::move(feed), fresh);
	case LogicalOperatorType::LOGICAL_DELETE:
		return DeleteRow(write.Cast<LogicalDelete>(), key_columns, std::move(feed), fresh);
	default:
...
SeamSplit SplitFeedIntoSeam(unique_ptr<LogicalOperator> feed, CrossingFragment &fragment, CrossingSource &source,
                            const SeamStop &stop, const FreshTableIndex &fresh) {
	if (!fragment.HasSeam()) {

...
void CrossingFragment::RemapTableIndices(const FreshTableIndex &fresh) {
	if (!plan) {
		return;
	}
	auto moved = duckdb::RemapTableIndices(*plan, fresh);
	auto at = [&](idx_t index) {
		auto it = moved.find(index);
		return it == moved.end() ? index : it->second;
	};
	table_index = at(table_index);
	for (auto &binding : floor_bindings) {
		binding.table_index = at(binding.table_index);
	}
	ResolveTypesAndText();
	VerifyInvariants();
}

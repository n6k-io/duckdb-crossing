...
void AlignEveryFragmentToItsScan(LogicalOperator &node, const FreshTableIndex &fresh) {
	...
	for (auto &child : node.children) {
		AlignEveryFragmentToItsScan(*child, fresh);
	}
}
...
void MoveCrossableWorkIntoFragments(unique_ptr<LogicalOperator> &plan, const FreshTableIndex &fresh) {
